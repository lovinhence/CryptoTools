...
def compare(src_exchange, dst_exchange, src_currency, dst_currency):
    ...
    try:
        ...
        ratio_src_dst_str = "%s %s,%s to %s:%f" % (
            src_currency, src_currency, src_exchange, dst_exchange, ratio_src_dst)
        ratio_dst_src_str = "%s %s,%s to %s:%f" % (
            src_currency, src_currency, dst_exchange, src_exchange, ratio_dst_src)
        print(ratio_src_dst_str)
        print(ratio_dst_src_str)
        if ratio_src_dst > 1.02:
            alert(ratio_src_dst_str)
        if ratio_dst_src > 1.02:
...
def start_compare():
    ...
    for line in lines:
        ...
        # compare(src_exchange, dst_exchange, src_currency, dst_currency)
    # alert engine
    t = threading.Thread(target=alertEngine.mainLoop)
    t.start()
